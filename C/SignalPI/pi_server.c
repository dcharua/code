...
double computePI(unsigned long int iterations){
  ...
  //Loop to calulate PI and check POLLIN for client exit signal
  for (counter = 0; counter<iterations && !interrupted; counter++){
    fflush(stdout);
     poll_result = poll(&test_fd, 1, timeout);
     // if the timeout finished without event calcule PI
     if (poll_result == 0){
       result += sign * (4.0/divisor);
       sign *= -1;
       divisor += 2;
     }
     //if there is something to read in FD (client sent stop msn) set interrupted 1 to break from loop
     else if (poll_result > 0){
       if (test_fd.revents && POLLIN){
         printf("exit signal recived from client in poll\n");
         interrupted = 1;
         break;
       }
     }
